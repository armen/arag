...
function smarty_function_thumb_imp($params, &$smarty) {/*{{{*/
    ...
    if (isset($params['cache_url'])) {
        $_CONFIG['cache_url'] = $params['cache_url'];
    } else {
        $_CONFIG['cache_url'] = $_CONFIG['cache'];
    }
    ...
    $_DST['file_url'] = $_CONFIG['cache_url'].$tmp_filename."_".$_SRC['hash'].$_CONFIG['types'][$_DST['type']];
	...
	if (empty($params['html'])) $_RETURN['img'] = '<img src="'.$_DST['file_url'].'" '.$params['html'].' '.$_DST['string'].' alt="" title="" />';
	else $_RETURN['img'] = '<img src="'.$_DST['file_url'].'" '.$params['html'].' '.$_DST['string'].' />';
