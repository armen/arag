...
{arag_block}
   ...
   {arag_form uri=help/backend/edit method="post"}
   <table border="0" dir="{dir}" width="100%">
        <tr>
            <td align="{right}" width="100">_("Title"):</td>
            <td><input type="text" name="title" value="{$help->title}" /></td>
        </tr>
        <tr>
            <td align="{right}">_("Message"):</td>
            <td>
                {arag_rte name="message" value=$help->message}
            </td>
        </tr>
        <tr>
            <td align="{right}">_("Dialog type"):</td>
            <td>
                {html_options name="type" options=$dialogs selected=$help->type}
            </td>
        </tr>
        <tr>
            ...
            <td>
                {foreach from=$groups item='group'}
                    <input id="group_{$group.id}" type="checkbox" name="groups[{$group.id}]" {if $group.isAllowed}checked="checked"{/if} />
